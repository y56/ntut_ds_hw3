...
/***************************************** ArrayStack *************************/
    ...
    for (int i = 0; i<9;i++)
    {
        datumItemPtrArray[i] = new DatumItem(3+i, (char)('c' + i));  // success  // check the contents of these nodes
        cout << (*(datumItemPtrArray[i])).key << datumItemPtrArray[i]->datumChar;
    }
    cout << endl;
    ...
    {
        cout << "i = " << i << endl;
        arrayStackPtr->push(  *(datumItemPtrArray[i])  );  // can't use arrayStackPtr.push()
        cout << "test getSize ==> "<< arrayStackPtr->getSize() << endl;
    }// Why I always get a some address ?? QQQQQQQQ
